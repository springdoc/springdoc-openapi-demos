...
public class SpringIntegrationWebFluxApplication {
	...
	public IntegrationFlow toUpperFlow() {
		return IntegrationFlows
				.from(WebFlux.inboundGateway("/conversions/upper")
	...
	public IntegrationFlow toUpperGetFlow() {
		return IntegrationFlows
				.from(WebFlux.inboundGateway("/conversions/pathvariable/{upperLower}")
						.requestMapping(r -> r.methods(HttpMethod.GET).params("toConvert"))
	...
	public IntegrationFlow toLowerFlow() {
		return IntegrationFlows.from(WebFlux.inboundGateway("/conversions/lower")
				.requestMapping(r -> r.methods(HttpMethod.POST).consumes("application/json"))
				.requestPayloadType(Foo.class)).<String>handle((p, h) -> p.toUpperCase()).get();
	}
	...
	public @ResponseBody Baz convert(@RequestBody Baz baz) {
		return baz;
	}
}